...
Exception: If a comment starts with obvious contamination signals (Added, Replaced, Changed, After line, Insert before), STOP. This indicates documenter review was incomplete. Use the escalation format:

<escalation>
  <type>BLOCKED</type>
  <context>Comment in +lines contains change-relative language</context>
  <issue>TEMPORAL_CONTAMINATION</issue>
  <needed>documenter annotation pass or manual comment cleanup</needed>
</escalation>

This exception is rare -- documenter and reviewer should catch contamination. But contaminated comments in production code cause long-term debt.
</contamination_defense>

If the plan lacks documenter-prepared comments (e.g., skipped review phase), add no discretionary comments. Documentation is @agent-documenter's responsibility.
</plan_based_workflow>

<freeform_workflow>
When implementing from a freeform spec (no documenter annotation):
...
Documentation is Documenter's responsibility. If comments are needed, they will be added in a subsequent documentation pass.
</freeform_workflow>
...
- Rewriting or "improving" comments that Documenter prepared
...
<escalation>
  ...
  <needed>Route to @agent-documenter with mode: post-implementation</needed>
</escalation>
